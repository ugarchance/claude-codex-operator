...
>
> Before replying, list live Claude sessions using the installed peer MCP bridge. Match the source session's PID and session UUID from `claude_session`; treat its supplied socket address as stale if it no longer matches. Send the concise result only to that verified session. With the bundled `claude_peers` bridge, call `send_message_to_claude` with `pid`, `session_id`, `message`, and `expect_reply: false`. If the peer bridge is unavailable, say so in this Codex task and do not claim direct delivery.
>
